...
let WarehouseService = class WarehouseService {
    ...
    prodRepo;
    prodAlmRepo;
    constructor(catRepo, medRepo, almRepo, sucRepo, prodRepo, prodAlmRepo) {
        this.catRepo = catRepo;
        this.medRepo = medRepo;
        this.almRepo = almRepo;
        this.sucRepo = sucRepo;
        this.prodRepo = prodRepo;
        this.prodAlmRepo = prodAlmRepo;
    }
    async onModuleInit() {
        const sucursalSeed = [
        ...
        for (const s of sucursalSeed) {
            ...
            if (!exists)
                await this.sucRepo.save(this.sucRepo.create(s));
        }
        const almacenSeed = [
        ...
        for (const a of almacenSeed) {
            ...
            if (!exists)
                await this.almRepo.save(this.almRepo.create(a));
        }
    }
    async getCategorias() {
    ...
    async createCategoria(dto) {
        ...
        if (exists)
            throw new common_1.ConflictException(`Categoría "${dto.NombreC}" ya existe.`);
        return this.catRepo.save(this.catRepo.create(dto));
    }
    async updateCategoria(id, dto) {
    ...
    async createAlmacen(dto) {
        return this.almRepo.save(this.almRepo.create(dto));
    }
    async updateAlmacen(id, dto) {
    ...
    async createSucursal(dto) {
        return this.sucRepo.save(this.sucRepo.create(dto));
    }
    async updateSucursal(id, dto) {
    ...
    async getProductos(search, categoriaId, almacenId) {
        ...
        if (almacenId) {
            qb.innerJoin(producto_almacen_entity_1.ProductoAlmacen, 'pa_filter', 'pa_filter.ID_Producto = p.ID_Producto AND pa_filter.ID_Almacen = :almId', { almId: almacenId });
        }
    ...
    async createProducto(dto) {
        if (dto.CodigoBarra) {
            ...
            if (exists)
                throw new common_1.ConflictException(`Código de barras "${dto.CodigoBarra}" ya está registrado.`);
        }
        const prod = await this.prodRepo.save(this.prodRepo.create(dto));
        const almacenes = await this.almRepo.find();
        if (almacenes.length > 0) {
            await this.prodAlmRepo.save(almacenes.map(a => ({ ID_Producto: prod.ID_Producto, ID_Almacen: a.ID_Almacen, Stock_Actual: 0 })));
        }
    ...
    async exportProductosPdf(search, categoriaId, almacenId, username) {
        ...
        const drawBadge = (txt, x, y, w, bg, tc, bc) => {
            doc.lineWidth(0.5);
            if (bc) {
        ...
        const drawPageHeader = () => {
            ...
            if (hasLogo) {
                try {
                    doc.image(logoPath, M, 11, { width: 56, height: 56 });
                }
                catch { }
            }
            const IX = PW - M - 200;
        ...
        const drawFooter = () => {
            ...
            doc.fillColor(LGRAY).fontSize(7).font('Helvetica')
                .text(`PARADISO Logistics © ${now.getFullYear()}  ·  ${items.length} producto${items.length !== 1 ? 's' : ''}`, M, FOOT_Y, { width: TW, align: 'center', lineBreak: false });
        };
        return new Promise((resolve, reject) => {
            ...
            doc.fillColor(GRAY).fontSize(8.5).font('Helvetica')
                .text('Listado Maestro de Artículos, Precios y Existencias por Almacén', M, tySubtitle, { width: TW, align: 'center', lineBreak: false });
            ...
            doc.fillColor(LGRAY).fontSize(7.5).font('Helvetica')
                .text(`Filtros aplicados:  Categoría (${catLabel}),  Almacén (${almLabel})`, M, tyFilter, { lineBreak: false });
            ...
            items.forEach((prod, idx) => {
                ...
                const total = (prod.productoAlmacenes ?? [])
                    .reduce((s, e) => s + Number(e.Stock_Actual), 0);
                ...
                if (factor > 1) {
                    ...
                    drawBadge(`${uds} Uds.`, bX, midY - 1, bW, uds > 0 ? GREENBG : REDBG, uds > 0 ? GREEN : RED);
                }
                else {
                    drawBadge(`${uds} Uds.`, bX, midY - 7, bW, uds > 0 ? GREENBG : REDBG, uds > 0 ? GREEN : RED);
                }
                y += ROW_H;
            });
            if (items.length === 0) {
                doc.fillColor(GRAY).fontSize(9).font('Helvetica')
                    .text('No se encontraron productos con los filtros seleccionados.', M, Y_ROW1 + 20, { width: TW, align: 'center', lineBreak: false });
            }
            drawFooter();
            doc.end();
        });
...
exports.WarehouseService = WarehouseService = __decorate([
    ...
    __param(1, (0, typeorm_1.InjectRepository)(u_medida_entity_1.UMedida)),
